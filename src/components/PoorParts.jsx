...
import { partsLookup, yesNoParts } from "../lib/constant";

const PoorPartComponent = ({ part }) => {
  return (
    <div className="border rounded-lg p-4 bg-red-500/10 border-red-100">
      <div className="space-y-1">
        <p className="font-medium text-sm">{partsLookup[part?.key]}</p>
...
export default function PoorParts({ poorParts, title }) {
    ...
    <div className="border rounded-xl border-red-100">
      ...
      <div className="p-4 sm:p-6 space-y-4">
        {poorParts.map((part, index) => {
          ...
          return yesNoParts.includes(part.key) ? (
            <PoorPartYesNo key={index} part={part} />
          ) : (
