...
const MediaComponent = ({ media }) => {
  ...
  return media?.photos.length === 0 && !media?.video ? null : (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 p-4 border-b">
      ...
      {media?.video && <VideoPlayer playbackId={videoSrc} onClose={onVideoClose} />}
    </div>
  );
};

const PresetCommentComponent = ({ k, value }) => {
  const isOptional = optionalParts.includes(k);
  return (
    <>
      <div>{partsLookup[k]}</div>
      ...
      {isOptional && !value?.available ? (
        ...
        <>
          ...
          <div className="md:col-span-2 space-y-2">
            <p>
              {value?.rating >= 3
                ? "Good condition. No damage. Paint condition good"
                : "Dents/scratches/paint fade detected"}
            </p>
          ...
          <div className="md:col-span-2 space-y-2">
            <p>
              {value.condition === "working"
                ? "Working"
                : value.condition === "not_working"
                ? "Not Working"
                : `Broken ${value.comment !== "" ? `. ${value.comment}` : ""}`}
            </p>
          </div>
        </>
      )}
...
const ParagraphComponent = ({ k, value }) => {
  return (
    <>
      <div>{partsLookup[k]}</div>
      ...
      <div className="md:col-span-2 space-y-2">
        <p>{value || "No Comment Provided"}</p>
      </div>
    </>
  );
};

export default function InspectionResult({ inspectionResult, title }) {
  return (
    <div>
                      ...
                      <div>
                        ...
                        {tableEntries.map(([key, value]) => (
                          <div key={key} className={styles.row}>
                            {presetParts.includes(key) ? (
                              <PresetCommentComponent k={key} value={value} />
                            ) : lightParts.includes(key) ? (
                              <LightComponent k={key} value={value} />
                            ) : yesNoParts.includes(key) ? (
                              <YesNoComponent k={key} value={value} />
                            ) : paragraphParts.includes(key) ? (
                              <ParagraphComponent k={key} value={value} />
                            ) : (
