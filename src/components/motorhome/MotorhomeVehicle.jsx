...
function getBodyTypeLabel(bodyType) {
  const bodyTypeLabels = {
    coach_built: "Coach Built Motorhome",
    campervan: "Campervan",
    class_a: "Class A Motorhome",
    class_b: "Class B Motorhome",
    class_c: "Class C Motorhome",
    slide_on: "Slide-on / Demountable",
    overcab_alcove: "Overcab / Alcove",
    low_profile: "Low-Profile Motorhome",
    a_class_integrated: "A-Class Integrated",
    panel_van_conversion: "Panel Van Conversion",
    bus_conversion: "Bus / Coaster Conversion",
  };
  return bodyTypeLabels[bodyType] || "N/A";
}

export default function MotorhomeVehicle({ vehicle }) {
    ...
    <div className="border rounded-xl sm:p-6 p-4 grid md:grid-cols-2 gap-5 lg:gap-10">
      <div>
        <h2 className="font-semibold text-xl sm:hidden mb-4">
          Premium Pre Purchase Motorhome and RV Inspection Report
        </h2>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 gap-4 ">
          <div>
            <p className="text-text text-sm">Base Vehicle (Make and Model)</p>
            <p className="font-medium">{vehicle?.makeModel || "N/A"}</p>
          </div>
          <div>
            <p className="text-text text-sm">Motorhome Brand / Conversion</p>
          ...
          <div>
            <p className="text-text text-sm">Rego</p>
            <p className="font-medium">{vehicle?.rego || "N/A"}</p>
          </div>
          <div>
            <p className="text-text text-sm">VIN</p>
          ...
          <div>
            ...
            <p className="font-medium">{getBodyTypeLabel(vehicle?.bodyType)}</p>
          </div>

          <div>
            <p className="text-text text-sm">GVM</p>
            <p className="font-medium">{vehicle?.gvm || "N/A"}</p>
          </div>
